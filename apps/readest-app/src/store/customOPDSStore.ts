...
/**
 * Stable identity for an OPDS catalog, derived from its URL (normalized:
 * trim + lower-case, so trailing-slash and case differences don't
 * fragment identity).
 */
...
interface OPDSStoreState {
  ...
  /** Patch a catalog's mutable fields. */
...
export const useCustomOPDSStore = create<OPDSStoreState>((set, get) => ({
  ...
  loadCustomOPDSCatalogs: async (_envConfig) => {
    try {
      ...
      // If backfill mutated anything, persist the fresh contentIds.
  ...
  saveCustomOPDSCatalogs: async (_envConfig) => {
    try {
      ...
      const { catalogs } = get();
      // Tombstoned entries stay in memory in case the user re-adds the
      // same URL (see `addCatalog`'s reincarnation handling above), but
      // they're stripped at the persistence boundary.
